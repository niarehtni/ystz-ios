...
@interface JCBRewardExplainVC () <UIWebViewDelegate>
@property (nonatomic, strong) UIScrollView *scrollView;
@property (nonatomic, strong) UIWebView *webView;
@end

@implementation JCBRewardExplainVC
...
- (void)getDataFromNetWorking {
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@/rest/article/%d", SGCommonURL, 653];
...
- (void)setupSubviews {
    ...
    _scrollView.backgroundColor = SGCommonBgColor;
    [self.view addSubview:_scrollView];
    
    self.webView = [[UIWebView alloc] init];
    _webView.frame = CGRectMake(0, 0, SG_screenWidth, _scrollView.SG_height);
...
-(void)webViewDidFinishLoad:(UIWebView *)webView{
    // 调整字号
    NSString *str1 = @"document.getElementsByTagName('body')[0].style.webkitTextSizeAdjust= '90%'";
    ...
    NSString * str = [NSString stringWithFormat:@"var script = document.createElement('script');"
                      ...
                      "}"
                      "}\";"
                      "document.getElementsByTagName('head')[0].appendChild(script);", SG_screenWidth - 25];
